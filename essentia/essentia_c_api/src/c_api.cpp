...
// Computes the key of a .raw file
const char* compute_key_from_file(const char* file_path) {
    try {
        std::vector<essentia::Real> audio = loadAudioBufferFromFile(file_path);
        std::string result = computeKey(audio);
        char* cstr = new char[result.size() + 1];
        std::copy(result.c_str(), result.c_str() + result.size() + 1, cstr);
        return cstr;
    } catch (const std::exception& e) {
        std::string error = std::string("Error: ") + e.what();
        char* cstr = new char[error.size() + 1];
        std::copy(error.c_str(), error.c_str() + error.size() + 1, cstr);
        return cstr;
    }
}

// Computes the key from a C float* buffer and its length
const char* compute_key_from_float_buffer(const float* buffer, int buffer_length) {
    try {
        std::vector<essentia::Real> audio = audioBufferToVector(buffer, buffer_length);
        std::string result = computeKey(audio);
        char* cstr = new char[result.size() + 1];
        std::copy(result.c_str(), result.c_str() + result.size() + 1, cstr);
        return cstr;
